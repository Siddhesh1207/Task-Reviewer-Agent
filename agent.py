...
# --- FastAPI App ---
app = FastAPI(title="Intelligent Task Reviewer Agent", version="5.0.0-sequential")
app.add_middleware(
    CORSMiddleware,
    allow_origins=["*"], allow_credentials=True, allow_methods=["*"], allow_headers=["*"],
)

# <-- MODIFIED: This function now ONLY generates the review and note. -->
def _run_review_and_note_logic(task_id: str, submission_text: str) -> dict:
    ...
    try:
        ...
        # <-- REMOVED: The next_task generation logic is gone from this function.
    ...
    # <-- MODIFIED: We save a placeholder for the next_task.
    ...
    # <-- MODIFIED: The return value is simpler now.
...
# <-- MODIFIED: This endpoint now calls the shorter logic function. -->
...
# <-- NEW: A separate endpoint to generate the next task. -->
...
def generate_next_task(review_id: str):
    ...
    try:
        # Use the original review data to generate the next task
        next_task = next_task_chain.invoke(review_record.get("review_data", {}))
        
        # Save the generated next_task back to the record for completeness
        reviews_collection.update_one(
            {"review_id": review_id},
            {"$set": {"next_task": next_task.model_dump()}}
        )
